...
-- 🛡️ [核: F*証明ロジックの継承] 
-- SecurityPolicy.fst で証明された「真理」を定義
...
-- Go Gatewayから送られてくる全情報を統合
data CheckRequest = CheckRequest 
    { userId :: Maybe String -- ユーザーID
    , cmd    :: String       -- 実行コマンド
    , karma  :: Int          -- 累積アタック回数（業）
    , token  :: Maybe String -- 検証用トークン
            ...
            if count >= maxKarma
                ...
                -- コマンドが正当であり、かつ業が臨界点に達していない場合
                    ...
                    then do
                        ...
                        liftIO $ putStrLn "OK"
                        liftIO $ putStrLn "---------------------------"
                        json $ VerdictResponse "VALID" validToken
                    ...
                    -- すでに所持しているトークンがF*の「真理」に合致するか確認
                        ...
                        Just t | isFStarValid (T.pack t) -> do
                            ...
                            json $ object ["status" .= ("OK" :: String), "token" .= validToken]
